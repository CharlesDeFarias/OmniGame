...
describe('piece packs', () => {
  it('maps normal pieces to the Kenney shape family in the gems and candy packs (decision #60)', () => {
    for (const pack of ['gems', 'candy'] as const) {
      for (const color of ALL_COLORS) {
        expect(pieceTextureKey({ kind: 'normal', color }, pack)).toBe(`img-shape-${color}`);
      }
    }
  });

  it('maps normal pieces to procedural music-<color> in the music pack (piece-pack feature)', () => {
    for (const color of ALL_COLORS) {
      expect(pieceTextureKey({ kind: 'normal', color }, 'music')).toBe(`music-${color}`);
    }
  });

  it('leaves specials pack-independent: img-sp-* aliases (procedural flat art) except propeller', () => {
  ...
  it('leaves blockers pack-independent on the crate aliases', () => {
    for (const pack of ['gems', 'candy', 'music'] as const) {
